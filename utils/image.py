...
def normalise_intensity(x,
    ...
    if type(x) is np.ndarray:
        ...
        # Normalise minmax
            ...
            x = min_out + (max_out - min_out) * (x - min_in) / (max_in - min_in + 1e-12)  # (!) multiple broadcasting)
    ...
    # for torch.Tensor
        ...
        # Normalise minmax
            ...
            x = min_out + (x - min_in) * (max_out - min_out) / (max_in - min_in + 1e-12)  # (!) multiple broadcasting)

        # Fixed ratio
        elif mode is "fixed":
            x = min_out + (x - min_in) * (max_out - min_out) / (max_in - min_in + 1e-12)
